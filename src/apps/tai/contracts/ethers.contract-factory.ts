...
import { TaiCollateralJoin__factory, TaiSafeJoin__factory, TaiSafeManager__factory } from './ethers';
...
export class TaiContractFactory extends ContractFactory {
  ...
  taiSafeManager({ address, network }: ContractOpts) {
    return TaiSafeManager__factory.connect(address, this.appToolkit.getNetworkProvider(network));
  }
}

export type { TaiCollateralJoin } from './ethers';
export type { TaiSafeJoin } from './ethers';
export type { TaiSafeManager } from './ethers';
