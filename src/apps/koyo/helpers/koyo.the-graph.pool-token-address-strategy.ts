...
type KoyoTheGraphPoolTokenDataStrategyParams = {
  subgraphUrl: string;
  minLiquidity?: number;
...
@Injectable()
export class KoyoTheGraphPoolTokenDataStrategy {
  constructor(@Inject(APP_TOOLKIT) private readonly appToolkit: IAppToolkit) {}

  @Cache({
    instance: 'business',
    key: (network: Network) => `studio-koyo-events-pool-token-addresses:${network}:koyo`,
    ttl: 5 * 60,
  })
  ...
  }: KoyoTheGraphPoolTokenDataStrategyParams) {
