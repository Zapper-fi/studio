...
import { MidasComptroller, MidasContractFactory, MidasCErc20Delegate } from '../contracts';

export type MidasPoolTokenDataProps = ExchangeableAppTokenDataProps & {
  supplyApy: number;
  borrowApy: number;
  liquidity: number;
  marketName?: string;
...
type MidasPoolTokenHelperParams<T = MidasComptroller, V = MidasCErc20Delegate> = {
  network: Network;
  appId: string;
  groupId: string;
  comptrollerAddress: string;
...
export class MidasPoolTokenHelper {
  ...
  async getTokens<T = MidasComptroller, V = MidasCErc20Delegate>({
  ...
  }: MidasPoolTokenHelperParams<T, V>) {
    ...
    const underlyings = await Promise.all(
      marketTokenAddressesRaw.map(async marketTokenAddressRaw => {
        ...
        const underlyingAddressRaw = await getUnderlyingAddress({ contract, multicall }).catch(err => {
          // if the underlying call failed, it's the wrapped native token
          const isWrappedNativeToken = err.message.includes('Multicall call failed for');
          if (isWrappedNativeToken) return ZERO_ADDRESS;
          throw err;
        });
    ...
    const tokens = await Promise.all(
      underlyings.map(async ({ underlyingTokenAddress, marketTokenAddress }) => {
        const erc20TokenContract = this.midasContractFactory.erc20({ address: marketTokenAddress, network });
        const contract = getTokenContract({ address: marketTokenAddress, network });
        ...
        const token: AppTokenPosition<MidasPoolTokenDataProps> = {
          type,
