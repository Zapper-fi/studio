    ...
    return this.appToolkit.helpers.contractPositionBalanceHelper.getContractPositionBalances(
      {
        ...
        resolveBalances: async ({ address, contractPosition, multicall }) => {
          ...
          const rewardToken = contractPosition.tokens.find(token => token.symbol === "xARGO" && token.metaType === MetaType.CLAIMABLE)!;
          ...
          return [
            ...
            drillBalance(rewardToken, rewardBalanceRaw.find(
              (token) => token.token === "0xb966B5D6A0fCd5b373B180Bbe072BBFbbEe10552",
            )!.amount.toString()),
            drillBalance(rewardToken2, rewardBalanceRaw.find(
              (token) => token.token === "0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23",
            )!.amount.toString())!,
          ];
        },
      }
