...
export class OptimismVelodromeStakingContractPositionFetcher implements PositionFetcher<ContractPosition> {
  ...
  async getPositions() {
    ...
    return this.appToolkit.helpers.singleStakingFarmContractPositionHelper.getContractPositions<VelodromeGauge>({
      network,
      appId,
      groupId,
      ...
      resolveIsActive: async ({ contract, multicall, rewardTokens }) => {
        const rates = await Promise.all(rewardTokens.map(async token => await multicall.wrap(contract).rewardRate(token.address)))
        return sum(rates.map(Number)) > 0;
      },
      resolveRois: async ({ contract, multicall, rewardTokens, stakedToken }) => {
        const baseROI = sum(await Promise.all(rewardTokens.map(async token => {
