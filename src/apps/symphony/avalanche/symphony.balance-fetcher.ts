...
export class AvalancheSymphonyBalanceFetcher implements BalanceFetcher {
  constructor(@Inject(APP_TOOLKIT) private readonly appToolkit: IAppToolkit) {}

  async getUserPositions(address: string) {
    const graphHelper = this.appToolkit.helpers.theGraphHelper;
    const data = await getOpenOrdersByUser(address.toLocaleLowerCase(), Network.AVALANCHE_MAINNET, graphHelper);
    return data.orders;
  }
