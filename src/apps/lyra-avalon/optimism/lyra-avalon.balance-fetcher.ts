...
export class OptimismLyraAvalonBalanceFetcher implements BalanceFetcher {
  ...
  async getOptionsBalances(address: string) {
    ...
    const markets = await Promise.all(response.markets.map(async market => {
      const tokenContract = this.contractFactory.optionToken({ address: market.optionToken.id.toLowerCase(), network })
    ...
    return this.appToolkit.helpers.contractPositionBalanceHelper.getContractPositionBalances({
      ...
      resolveBalances: async ({ contractPosition }) => {
        ...
        const userPosition = market.userPositions.find(position => Number(position.strikeId) === Number(strikeId) && position.optionType === Number(optionType))
        ...
        if (!strike) return [];
        const price = (OPTION_TYPES[optionType].includes('Call') ? strike.callOption : strike.putOption).latestOptionPriceAndGreeks.optionPrice;
        const balance = (Number(price) * Number(userPosition.amount) / 10 ** quoteToken.decimals).toString()

        // Return balance
        // Note: may not be totally accurate
        if (Number(optionType) >= 2) { // Short
