...
import { NereusFinanceDataProvider, NereusFinanceContractFactory } from '../contracts';
...
export type NereusFinanceLendingTokenDataProps = ExchangeableAppTokenDataProps & {
  apy: number;
...
export type NereusFinanceLendingTokenHelperParams<T = NereusFinanceDataProvider> = {
  appId: string;
  groupId: string;
  network: Network;
  ...
  resolveContract?: (opts: { contractFactory: NereusFinanceContractFactory; address: string }) => T;
...
export class NereusFinanceLendingTokenHelper {
  ...
  async getTokens<T = NereusFinanceDataProvider>({
    ...
    resolveContract = ({ contractFactory, address }) =>
      contractFactory.nereusFinanceDataProvider({ network, address }) as unknown as T,
    resolveReserveTokens = ({ contract }) =>
      (contract as unknown as NereusFinanceDataProvider).getAllReservesTokens().then(v => v.map(t => t[1])),
    resolveReserveTokenAddresses = ({ contract, multicall, reserveTokenAddress }) =>
      multicall.wrap(contract as unknown as NereusFinanceDataProvider).getReserveTokensAddresses(reserveTokenAddress),
    resolveReserveData = ({ contract, multicall, reserveTokenAddress }) =>
      multicall.wrap(contract as unknown as NereusFinanceDataProvider).getReserveData(reserveTokenAddress),
    resolveReserveConfigurationData = ({ contract, multicall, reserveTokenAddress }) =>
      multicall.wrap(contract as unknown as NereusFinanceDataProvider).getReserveConfigurationData(reserveTokenAddress),
  }: NereusFinanceLendingTokenHelperParams<T>): Promise<AppTokenPosition<NereusFinanceLendingTokenDataProps>[]> {
    ...
    const tokens = await Promise.all(
      reserveTokenAddresses.map(async reserveTokenAddress => {
        ...
        const token: AppTokenPosition<NereusFinanceLendingTokenDataProps> = {
