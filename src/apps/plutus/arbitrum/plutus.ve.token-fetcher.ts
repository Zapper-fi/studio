...
import { ADDRESSES } from './plutus.pools.contract-position-fetcher'
...
export class ArbitrumPlutusVeTokenFetcher implements PositionFetcher<AppTokenPosition> {
  ...
  async getVePosition(address: string, baseAddress: string) {
    ...
    const [supplyRaw, decimals, symbol] = await Promise.all([veToken.totalSupply(), veToken.decimals(), veToken.symbol()]);
  ...
  async getPositions() {
    const plsJones = await this.getVePosition(ADDRESSES.plsJones, ADDRESSES.jones)
    const plsDopex = await this.getVePosition(ADDRESSES.plsDpx, ADDRESSES.dpx)
