...
const groupId = LIZ_TEST_DEFINITION.groups.jar.id;
...
export class EthereumLizTestJarTokenFetcher implements PositionFetcher<AppTokenPosition> {
  constructor(
  ...
  async getPositions() {
    ...
    const tokens = await Promise.all(
      jarAddresses.map(async jarAddress => {
        ...
        const contract = this.lizTestContractFactory.pickleJar({ address: jarAddress, network });
        ...
        const underlyingTokenContract = this.lizTestContractFactory.pickleJar({
