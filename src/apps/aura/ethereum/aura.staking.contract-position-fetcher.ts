...
@PositionTemplate()
export class EthereumAuraStakingContractPositionFetcher extends SingleStakingFarmTemplateContractPositionFetcher<AuraBaseRewardPool> {
  groupLabel = 'Staked auraBAL';
  ...
  async getFarmDefinitions(): Promise<AuraSingleStakingFarmDefinition[]> {
    const stakingPoolDefinition = await this.rewardPoolResolver.getStakingDefinition();

    return stakingPoolDefinition;
  }
