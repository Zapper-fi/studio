  ...
  async getLoanBalances(address: string) {
    return this.appToolkit.helpers.contractPositionBalanceHelper.getContractPositionBalances({
      ...
      resolveBalances: async ({ address, contractPosition, multicall }) => {
        ...
        const contract = this.inverseFirmContractFactory.simpleMarket({ address: contractPosition.address, network });

        const personalEscrow = await multicall.wrap(contract).escrows(address);

        const escrowContract = this.inverseFirmContractFactory.simpleEscrow({ address: personalEscrow, network });
