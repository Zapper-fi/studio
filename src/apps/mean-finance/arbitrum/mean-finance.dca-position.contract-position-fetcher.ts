...
import { getPositions, getUserPositions } from '../helpers/graph';
...
@PositionTemplate()
export class ArbitrumMeanFinanceDcaPositionContractPositionFetcher extends CustomContractPositionTemplatePositionFetcher<
...
> {
  ...
  async getDefinitions() {
    const allPositions = await Promise.all(
      POSITIONS_VERSIONS.map(async version => {
        const positionData = await getPositions(this.network, this.appToolkit.helpers.theGraphHelper, version);
        ...
        return uniqueDirectionsWithLiquidity.map(v => ({ address: HUB_ADDRESS[version][this.network]!, ...v }));
      }),
    );
  ...
  async getBalances(address: string): Promise<ContractPositionBalance<MeanFinanceDcaPositionDataProps>[]> {
    ...
    const allUserPositions = await Promise.all(
      POSITIONS_VERSIONS.map(async version => {
        const dcaHubAddress = HUB_ADDRESS[version][this.network]!;
        if (!dcaHubAddress) return [];

        const graphHelper = this.appToolkit.helpers.theGraphHelper;
        const userPositionsData = await getUserPositions(address.toLowerCase(), this.network, graphHelper, version);
        ...
        const userPositions = await Promise.all(
          userPositionsData.positions.map(async userPos => {
            ...
            if (hasYield && version === POSITION_VERSION_4) {
              const transformerAddress = TRANSFORMER_REGISTRY_ADDRESS[POSITION_VERSION_4][this.network]!;
