...
import { UniswapFactory, UniswapPair } from '~apps/uniswap-v2';
...
export type HakuswapPoolTokenDataProps = {
  liquidity: number;
  fee: number;
  volume: number;
...
export type HakuswapPoolTokenHelperParams<T = UniswapFactory, V = UniswapPair> = {
  network: Network;
  appId: string;
  groupId: string;
...
export class HakuswapPoolTokenHelper {
  ...
  async getTokens<T = UniswapFactory, V = UniswapPair>({
  ...
  }: HakuswapPoolTokenHelperParams<T, V>) {
    ...
    const poolTokens = await Promise.all(
      compact(poolTokensWithAddresses).map(async ({ address, token0Address, token1Address }) => {
        ...
        const supply = poolNotUsingDecimals.includes(address) ? Number(supplyRaw) : Number(supplyRaw) / 10 ** decimals;
        ...
        const poolToken: AppTokenPosition<HakuswapPoolTokenDataProps> = {
          type,
          address,
          network,
          appId,
