...
export class AvalancheRoboVaultVaultTokenFetcher implements PositionFetcher<AppTokenPosition> {
  constructor(
  ...
  async getPositions() {
    ...
    const ethData = data.filter(({ chain, status }) => chain === 'avalanche' && status === 'active');
