...
import { IronBankCToken } from '../contracts';

import { IronBankSupplyTokenDataProps } from './ironBank.supply.token-helper';

type IronBankLendingBalanceHelperParams<T> = {
  address: string;
  network: Network;
  appId: string;
...
@Injectable()
export class IronBankLendingBalanceHelper {
  constructor(@Inject(APP_TOOLKIT) private readonly appToolkit: IAppToolkit) {}

  async getBalances<T = IronBankCToken>({
    address,
    network,
    appId,
    supplyGroupId,
    ...
    getBalanceRaw,
    getBorrowBalanceRaw,
  }: IronBankLendingBalanceHelperParams<T>) {
    const multicall = this.appToolkit.getMulticall(network);

    const [supplyTokens, borrowPositions] = await Promise.all([
      this.appToolkit.getAppTokenPositions<IronBankSupplyTokenDataProps>({
        appId,
        groupIds: [supplyGroupId],
        network,
      }),
      this.appToolkit.getAppContractPositions<IronBankSupplyTokenDataProps>({
        appId,
