...
export type IronBankSupplyTokenDataProps = {
  supplyApy: number;
  borrowApy: number;
  liquidity: number;
  supply: number;
...
type IronBankBorrowContractPositionHelperParams = {
  network: Network;
  appId: string;
  groupId: string;
...
@Injectable()
export class IronBankBorrowContractPositionHelper {
  constructor(
    @Inject(IronBankContractFactory) private readonly contractFactory: IronBankContractFactory,
    @Inject(APP_TOOLKIT) private readonly appToolkit: IAppToolkit,
  ) {}

  async getPositions({ network, appId, groupId, supplyGroupId }: IronBankBorrowContractPositionHelperParams) {
    const appTokens = await this.appToolkit.getAppTokenPositions<IronBankSupplyTokenDataProps>({
      appId,
    ...
    const promisedPositions = appTokens.map(async appToken => {
      const contract = this.contractFactory.ironBankCToken({ network, address: appToken.address });

      // IronBank has a `getCash` method which returns the total liquidity
      ...
      const contractPosition: ContractPosition<IronBankSupplyTokenDataProps> = {
