...
import { IronBankSupplyTokenDataProps } from './ironBank.borrow.contract-position-helper';

@Injectable()
export class IronBankTvlHelper {
   ...
   * @description Infer the total liquidity of a ironBank like app by inferring
  ...
  public async getLiquidityBasedOnBorrowedPositions({
    ...
    const borrow = await this.appToolkit.getAppContractPositions<IronBankSupplyTokenDataProps>({
      appId,
      groupIds,
      network,
    });
   ...
   * @param config.groupIds- List of group ids that target BORROWED contract positions.
   * @param config.network- Network in which we wish to fetch the data from
   * @description Infer the total supply of a ironBank like app by inferring
