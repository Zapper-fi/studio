...
type IronBankSupplyTokenHelperParams<T = IronBankComptroller, V = IronBankCToken> = {
  network: Network;
  appId: string;
  groupId: string;
...
export class IronBankSupplyTokenHelper {
  ...
  async getTokens<T = IronBankComptroller, V = IronBankCToken>({
  ...
  }: IronBankSupplyTokenHelperParams<T, V>) {
    ...
    const tokens = await Promise.all(
      marketTokenAddressesRaw.map(async marketTokenAddressRaw => {
        ...
        const token: AppTokenPosition<IronBankSupplyTokenDataProps> = {
          type,
          address,
          network,
          appId,
