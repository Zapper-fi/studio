...
export class PolygonBadgerVaultTokenFetcher implements PositionFetcher<AppTokenPosition> {
  ...
  async getVaultDefinitions() {
    const { data } = await Axios.get<BadgerApiTokensResponse>('https://api.badger.com/v2/tokens?chain=polygon');
