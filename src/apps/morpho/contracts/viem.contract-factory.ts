...
export class MorphoViemContractFactory {
  ...
  morphoAdaptiveCurve({ address, network }: ContractOpts) {
    return MorphoAdaptiveCurve__factory.connect(address, this.appToolkit.getViemNetworkProvider(network));
  }
