...
export class CronosVvsFinanceFarmV2ContractPositionFetcher implements PositionFetcher<ContractPosition> {
  constructor(
    ...
    private readonly poolCacheManager: CronosVvsFinancePoolAddressCacheManager,
  ) {}
  ...
  async getPositions() {
    ...
    const craftsmanV2Contract = multicall.wrap(this.contractFactory.vvsCraftsmanV2({ address, network }));
      ...
      await Promise.all(
        pools.map(async pool => {
          ...
          const rewards = await Promise.all(
            rewarderAddresses.map(async rewardAddress => {
              ...
              const { allocPoint, lastRewardTime, accRewardPerShare } = await rewarderContract.poolInfo(pool.poolId);
              return {
    ...
    const positions =
      await this.appToolkit.helpers.masterChefContractPositionHelper.getContractPositions<VvsCraftsmanV2>({
        network,
        groupId,
        appId,
        address,
        ...
        resolvePoolLength: () => Math.max(...Object.keys(poolRecords).map(i => parseInt(i))) + 1,
