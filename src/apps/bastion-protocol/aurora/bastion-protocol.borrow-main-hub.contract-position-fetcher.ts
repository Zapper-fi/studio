...
const groupId = BASTION_PROTOCOL_DEFINITION.groups.borrowMainHub.id;
const network = Network.AURORA_MAINNET;

@Register.ContractPositionFetcher({ appId, groupId, network })
export class AuroraBastionProtocolBorrowMainHubContractPositionFetcher implements PositionFetcher<ContractPosition> {
  constructor(
  ...
  async getPositions() {
    return this.bastionBorrowContractPositionHelper.getPositions({
      ...
      appId,
      groupId,
      supplyGroupId: BASTION_PROTOCOL_DEFINITION.groups.supplyMainHub.id,
    });
  }
}
