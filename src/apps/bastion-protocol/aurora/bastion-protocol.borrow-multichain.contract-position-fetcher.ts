...
const groupId = BASTION_PROTOCOL_DEFINITION.groups.borrowMultichain.id;
const network = Network.AURORA_MAINNET;

@Register.ContractPositionFetcher({ appId, groupId, network })
export class AuroraBastionProtocolBorrowMultichainContractPositionFetcher implements PositionFetcher<ContractPosition> {
  constructor(
  ...
  async getPositions() {
    return this.bastionBorrowContractPositionHelper.getPositions({
      ...
      appId,
      groupId,
      supplyGroupId: BASTION_PROTOCOL_DEFINITION.groups.supplyMultichain.id,
    });
  }
}
