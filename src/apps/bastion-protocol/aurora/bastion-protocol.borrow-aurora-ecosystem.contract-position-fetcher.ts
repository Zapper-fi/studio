...
const groupId = BASTION_PROTOCOL_DEFINITION.groups.borrowAuroraEcosystem.id;
const network = Network.AURORA_MAINNET;

@Register.ContractPositionFetcher({ appId, groupId, network })
export class AuroraBastionProtocolBorrowAuroraEcosystemContractPositionFetcher
  implements PositionFetcher<ContractPosition>
{
  ...
  async getPositions() {
    return this.bastionBorrowContractPositionHelper.getPositions({
      ...
      appId,
      groupId,
      supplyGroupId: BASTION_PROTOCOL_DEFINITION.groups.supplyAuroraEcosystem.id,
    });
  }
}
