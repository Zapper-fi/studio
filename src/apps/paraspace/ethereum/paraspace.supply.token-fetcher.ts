...
import { ParaspacePToken } from '../contracts/ethers/ParaspacePToken';

@PositionTemplate()
export class EthereumParaspaceSupplyTokenFetcher extends AppTokenTemplatePositionFetcher<
  ParaspacePToken,
...
> {
  ...
  getContract(address: string): ParaspacePToken {
    return this.contractFactory.paraspacePToken({ address: address, network: this.network });
  }
  ...
  async getUnderlyingTokenDefinitions({ contract }: GetUnderlyingTokensParams<ParaspacePToken>) {
  ...
  getTokenAddress(reserveTokenAddressesData: ParaspaceReserveTokenAddressesData): string {
    return reserveTokenAddressesData.pTokenAddress;
  }
}
